...
pub const TRANSLATIONS: &[TranslationInfo] = &[TranslationInfo {
    code: "TORAH",
...
pub struct BrowserState {
    ...
    /// Empty vec means use canonical English names.
...
impl BrowserState {
    ...
    pub fn restore(&mut self, saved: &crate::store::state::SessionState) {
        ...
        self.chapter_list
            .select(Some((self.selected_chapter - 1) as usize));
    ...
    /// Returns true if the current translation is available offline.
    /// Returns true if the translation has local data.
    ...
    pub fn download_progress(&self) -> Option<(usize, usize)> {
        self.download
            .as_ref()
            .map(|dl| (dl.completed.load(Ordering::Relaxed), dl.total))
    }
...
pub fn render_browser(
    ...
    let outer_block = Block::default()
        .title(Line::from(vec![
            Span::styled(" torah", Style::default().fg(theme.accent).bold()),
...
fn render_chapters_panel(frame: &mut Frame, area: Rect, state: &mut BrowserState, theme: &Theme) {
    let is_active =
        state.active_panel == Panel::Chapters && matches!(state.search, SearchMode::Off);
    let block = Block::default()
        .title(Span::styled(
            " Ch ",
            Style::default()
...
fn render_scripture_panel(frame: &mut Frame, area: Rect, state: &mut BrowserState, theme: &Theme) {
    let is_active =
        state.active_panel == Panel::Scripture && matches!(state.search, SearchMode::Off);
...
fn render_search_results_panel(
    ...
    let (query, results, list_state) = match &mut state.search {
        SearchMode::Active {
            ...
            results,
            list_state,
        } => (query.clone(), results, list_state),
        _ => return,
    };
