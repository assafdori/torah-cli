...
pub fn remove_complete_marker(translation: &str) {
    if let Some(dir) = cache_dir() {
        let _ = fs::remove_file(dir.join(translation.to_uppercase()).join(".complete"));
    }
}
...
pub fn load_book_names(translation: &str) -> Option<Vec<String>> {
    let dir = cache_dir()?;
    let data = fs::read_to_string(dir.join(translation.to_uppercase()).join("books.json")).ok()?;
    serde_json::from_str(&data).ok()
}
...
pub fn search(translation: &str, query: &str) -> Vec<SearchResult> {
    ...
    for book in BOOKS {
        for ch_num in 1..=book.chapters {
            let path = trans_dir.join(format!("{}_{}.json", book.id, ch_num));
...
/// Total number of chapters in the configured canon.
...
pub fn verify_and_mark_complete(translation: &str) {
    if is_fully_cached(translation) || translation.eq_ignore_ascii_case("TORAH") {
        return;
    }
    ...
    for book in BOOKS {
        for ch in 1..=book.chapters {
            if trans_dir.join(format!("{}_{}.json", book.id, ch)).exists() {
                cached += 1;
            }
        }
    }
