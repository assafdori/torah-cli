...
const REPO: &str = "assafdori/torah-cli";
...
pub async fn run_update(check_only: bool) -> Result<(), String> {
    ...
    if check_only {
        println!("Run `torah update` to install v{}.", latest);
        return Ok(());
    }
    ...
    let url = format!(
        "https://github.com/{}/releases/download/v{}/torah-{}.{}",
        REPO, latest, target, ext
    );

    println!("Downloading torah v{} for {}...", latest, target);
    ...
    let archive_path = temp_dir.join(format!("torah-update.{}", ext));
    fs::write(&archive_path, &bytes).map_err(|e| format!("Failed to write temp file: {}", e))?;

    // Extract
    let extract_dir = temp_dir.join("torah-update-extract");
    let _ = fs::remove_dir_all(&extract_dir);
    fs::create_dir_all(&extract_dir).map_err(|e| format!("Failed to create temp dir: {}", e))?;
    ...
    let binary_name = if cfg!(windows) { "torah.exe" } else { "torah" };
