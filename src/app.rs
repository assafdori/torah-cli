...
impl App {
    ...
    pub async fn run(mut self, mut terminal: DefaultTerminal) -> std::io::Result<()> {
        ...
        // Fetch localized book names for non-TORAH translations
        let localized_books = if !translation.eq_ignore_ascii_case("TORAH") {
            self.resolver
                .get_book_names(translation)
                .await
                .unwrap_or_default()
        } else {
            Vec::new()
        ...
        // Start background cache download for non-TORAH translations
    ...
    fn start_cache_download(&mut self) {
        if let AppMode::Browser(ref mut state) = self.mode {
            ...
            // TORAH is bundled, no need to download
    ...
    /// Live search only for offline translations (TORAH bundled or fully cached).
    ...
    /// Run search using bundled TORAH (instant), cached data, or remote provider.
    async fn live_search(&mut self) {
        ...
        // Search: instant for TORAH, resolver path for others
        let search_results = if translation.eq_ignore_ascii_case("TORAH") {
            kjv::search(&query)
        } else {
            self.resolver
